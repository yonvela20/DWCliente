 ...
 $(function(){
    ...
    //Esto yo creo que no hace nada hM
...
function dobleClickItems(){
    ...
    if($cantStock == 0){
        $(".item").off("dblclick", dobleClickItems);
        console.log("Hago el off");
    }
...
function compra($this, $cantStock, $numItems, $precioTotal, $precio){
    ...
    if($cantStock == 0){
        ...
        $(".stock", $this).remove("agotado");
    }
}

//Funcion que clona items (se usa despues de hacer una compra)
...
//Funcion para borrar items asociada al enlace de borrar de los items del carrito 
function borrado(){
    const $divCarrito = $("#cart_items"); //Pillamos el contenedor del carrito
    ...
    /* if(stock > 0){
        $("#"+idItem).removeClass('agotado');
    } */
    return false;
}


//Funcion que se encarga de eliminar los productos del carrito y actualizar labels y de más 
//llamada en laf funcion borrado()
